...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        //The api is read every 3 minutes so as to get the lastest information.
        var _ = Timer.scheduledTimer(timeInterval: 180.0, target: self,selector: #selector(self.loadLocationWeathers),userInfo: nil,repeats: true)
    ...
    @objc func loadLocationWeathers(){
        ...
        group.notify(queue: DispatchQueue.main) {
            ...
            if let city = self.lblCity.text{
                let indexTodisplay = Helper.getIndexToDisplay(currentLocation: city, fromLocation: locations)
                ...
                updatedTime = "\(hour):\(minutes):\(seconds)"
                
                self.loadView(indexTodisplay: indexTodisplay)
            }
    ...
    func loadView(indexTodisplay:Int){
        ...
        if let locationWeather = location.locationWeathers{
            ...
            let retrievedDate = Helper.convertDatefrom(string: locationWeather.applicable_date ?? " ")
            self.lblApplicableDate?.text = retrievedDate
            
         
            self.lblLastUpdated?.text =  "Last Updated at \(updatedTime)"
    ...
    func cutsomizeLabelColor(color:UIColor)->Void{
        ...
        self.lblLastUpdated?.textColor = color
    }
